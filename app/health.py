...
def calculate_health(repository: dict, commits: dict, pulls: dict, issues: dict) -> dict:
    ...
    signals = {
        ...
        "contributor_distribution": 100 if not total else max(0, 100 - max(0, top_share - 40) * (100 / 60)),
    ...
    return {
        ...
        "signals": details,
        "recommendations": recommendations(signals, pulls, issues, total, top_share, inactivity),
    }


def recommendations(signals, pulls, issues, commits, top_share, inactivity) -> list[dict]:
    result = []
